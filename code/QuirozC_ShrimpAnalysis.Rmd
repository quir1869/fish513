...
```{r analyze OBIS data}
# Get list of exploited decapods (from fishbase)
...
all_decapods_obis <- occurence(scientificname = "Decapoda")
...
# Continents spatial extent
continents <- ne_countries(scale = "medium", returnclass = "sf", continent = c("Africa", "Antarctica", "Asia", "Europe", "North America", "Oceania", "South America"))
 
# Convert the dataset to an sf object
data_sf <- st_as_sf(exploited_obis, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)

# Perform the spatial join to determine the continent
data_with_continent <- st_join(data_sf, continents, join = st_intersects)

# Pipe occurence into this
occurrence_data <- exploited_obis %>%  
mutate(individualCount = as.numeric(individualCount), 
       date_year = make_date(date_year, 1, 1), iso3 = countrycode(country, origin = "country.name", destination = "iso3c")) %>%
  exploreARTIS::add_region("iso3", region.col.name = "region") %>%
  filter(!is.na(individualCount), !is.na(date_year)) %>%
...
# Pipe occurence into this
occurrence_data <- data_with_continent %>%  
mutate(individualCount = as.numeric(individualCount), 
       date_year = make_date(date_year, 1, 1)) %>%
  filter(!is.na(individualCount), !is.na(date_year)) %>%
  rename(date = "date_year", region = "continent.y") %>%
  group_by(date, region) %>%
  summarize(count = sum(individualCount)) %>%
  ungroup()
...
occurrence_data %>%
      ggplot(aes(x = date, y = log(count), color = region)) +
      geom_line() +
      theme_light() +
      labs(x = "", y = "Decapoda count", color = "Region") +
      geom_point() +
      scale_color_manual(values = pnw_palette("Bay",7)) +
      facet_wrap(~ region) +
      guides(color = "none")

# Occurece data trends, globally (decapods)
...
(decapoda_family_trends <- obis_clean %>%
